...
public class VentanaInsertarAutobus extends Controlador{
	...
	private void insertar() {
		...
		if (tfMatricula.getStyleClass().contains("valido") && tfMarca.getStyleClass().contains("valido") && tfModelo.getStyleClass().contains("valido")) {
			try {
				vistaGrafica.getControlador().insertar(new Autobus(tfMarca.getText(), tfModelo.getText(), Integer.parseInt(tfPlazas.getText()), tfMatricula.getText()));
				Dialogos.mostrarDialogoInformacion("Insercion correcta", "Autobús insertado correctamente", getEscenario());
