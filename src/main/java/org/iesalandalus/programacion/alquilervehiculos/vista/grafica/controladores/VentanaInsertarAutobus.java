...
public class VentanaInsertarAutobus extends Controlador {
	...
	private void deshabilitarBoton(String er, TextField campoTexto) {
		...
		boolean invalido = !(tfMatricula.getStyleClass().contains("valido") && tfMarca.getStyleClass().contains("valido")
				&& tfModelo.getStyleClass().contains("valido") && tfPlazas.getStyleClass().contains("valido"));
	...
	private void insertar() {
			...
			try {
				vistaGrafica.getControlador().insertar(new Autobus(tfMarca.getText(), tfModelo.getText(),
						Integer.parseInt(tfPlazas.getText()), tfMatricula.getText()));
				Dialogos.mostrarDialogoInformacion("Insercion correcta", "Autobús insertado correctamente",
						getEscenario());
